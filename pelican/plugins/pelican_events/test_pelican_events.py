...
class TestStripHtmlTags(TestCaseSet):
    ...
    def test_strip_html_tags(self):
        ...
        for test_num, test_case in enumerate(TEST_CASES[self.__class__.case_set()]):
            with self.subTest(type=self.__class__.case_set(), number=test_num, text=test_case['in']):
                self.assertEqual(pelican.plugins.pelican_events.strip_html_tags(test_case['in']), test_case['out'])
